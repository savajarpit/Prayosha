...
const AdminOrders = () => {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_APIS}/api/v1/auth/all-orders`);
      setOrders(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleChange = async (orderId, value) => {
    try {
      const { data } = await axios.put(`${import.meta.env.VITE_APIS}/api/v1/auth/order-status/${orderId}`, {
        status: value,
      });
      getOrders();
    } catch (error) {
